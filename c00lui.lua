...
function Window:_createGui()
    ...
    leftButton.MouseButton1Click:Connect(function()  -- NOVO: Usei MouseButton1Click pra ser mais moderno
        self:PreviousPage()
    end)
    ...
    self.toggleButton.MouseButton1Click:Connect(function()  -- NOVO: MouseButton1Click
        self:Toggle()
    end)
end

-- NOVO: AddPage com nome custom (pra abas nomeadas como na original)
function Window:AddPage(name)
    local pageIndex = #self.pages + 1
    local pageFrame = Instance.new("Frame")
    pageFrame.Name = name  -- NOVO: Usa o nome passado
...
function Page:AddSection(name, config)
    ...
    sectionFrame.Position = UDim2.new((sectionIndex == 1 and 0 or 0.5), (sectionIndex == 1 and 0 or -3), 0, 0)  -- NOVO: Corrigi o -3 pra não sobrepor
    sectionFrame.Size = UDim2.new(0.5, -3, 1, 0)  -- NOVO: Tamanho fixo pra colunas lado a lado como na original
...
-- NOVO: AddButton com callback OnClick e mais configs
function Section:AddButton(name, config)
    ...
    button.Size = UDim2.new(1, 0, 0, config.Height or 30)  -- NOVO: Largura full na seção
    ...
    if config.OnClick then
        button.MouseButton1Click:Connect(config.OnClick)  -- NOVO: Callback como função
    end
...
function Section:AddToggle(name, config)
    ...
    checkBox.MouseButton1Click:Connect(function()  -- NOVO: MouseButton1Click
        state = not state
...
-- NOVO: AddTextInput melhorado com setText e OnChange
function Section:AddTextInput(placeholder, config)
    ...
    input.Text = config.DefaultText or ""  -- NOVO: Suporte pra texto default
    ...
    return {
        ...
        setText = function(text) input.Text = text end  -- NOVO: Função pra setar texto
    }
end
